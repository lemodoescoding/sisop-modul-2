...
void activityLog(const char* judul, const char* genre, const char* penulis) {
    ...
    fprintf(log, "[%2d-%2d-%4d %2d:%2d:%2d] %s: %s telah dipindahkan ke %s\n", waktu->tm_mday, waktu->tm_mon + 1, waktu->tm_year + 1900, waktu->tm_hour, waktu->tm_min, waktu->tm_sec, penulis, judul, genre);
    fclose(log);
...
void* sortirfilm(void* argv) {
    ...
    struct dirent* entri;
    char folderbaru[100];
    sprintf(folderbaru, "film/Film%s", genre);
    ...
    while ((entri = readdir(folder)) != NULL) {
        ...
        if (len > 4 && strcmp(entri->d_name + len - 4, ".jpg") == 0) {
            ...
            if (strcmp(genre_file, genre) == 0) {
                ...
                if (strcmp(genre, "horror") == 0) horror++;
                else if (strcmp(genre, "animasi") == 0) animasi++;
                else if (strcmp(genre, "drama") == 0) drama++;
            }
        }
    }
